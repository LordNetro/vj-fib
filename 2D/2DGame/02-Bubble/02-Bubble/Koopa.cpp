...
void Koopa::init(const glm::ivec2& tileMapPos, ShaderProgram& shaderProgram)
{
	...
	sprite->addKeyframe(STAND_DEAD, glm::vec2(0.3125f, 0.f));

	sprite->setAnimationSpeed(MOVE_DEAD, 8);
	sprite->addKeyframe(MOVE_DEAD, glm::vec2(0.3125f, 0.f));
...
void Koopa::update(int deltaTime)
{
	...
	if (isPushed) maxSpeedX = 4.f;
	glm::vec2 futurePos = posKoopa;  // Creamos una posición futura para verificar colisiones antes de mover al koopa.
	...
	if (!isDying) {
		...
		if (Movement == LEFT)
		{
			...
			if (!isPushed && speedX < 0 && sprite->animation() != MOVE_LEFT)
		...
		if (sprite->animation() != STAND_DEAD) sprite->changeAnimation(STAND_DEAD);
	}
