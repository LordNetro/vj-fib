...
bool TileMap::collisionMoveRight(const glm::ivec2 &pos, const glm::ivec2 &size) const //BUG: Parece que detecta colision pero sigue
{
	int x, y0, y1;
...
bool TileMap::collisionMoveUp(const glm::ivec2 &pos, const glm::ivec2 &size, int *posY, int &blockType, bool breakable, glm::vec2 &blockPos) const
{
	...
	x0 = (pos.x + 1) / tileSize; // Esquina izquierda del sprite
    x1 = (pos.x + size.x - 1) / tileSize; // Esquina derecha del sprite
    y = (pos.y) / tileSize; // Parte superior del sprite
		...
		if (map[(y-1)*mapSize.x + x] != 0) // Comprobamos si el tile no es vacío (es sólido)
        {
            ...
            if (tileSize * (y + 1) - pos.y <= 6) // Verificamos si la diferencia es menor a 4 píxeles
            {
                ...
                *posY = tileSize * y + 1; // Ajustamos la posición en Y para evitar la colisión
				if (blockType == 3 && breakable) map[(y - 1) * mapSize.x + x] = 0;
